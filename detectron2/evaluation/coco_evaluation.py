...
class COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(
        ...
        *,
        ...
        self.unseen_cnames = unseen_cnames
        self.seen_cnames = seen_cnames
        self.all_cnames = all_cnames
    ...
    def _derive_coco_results(self, coco_eval, iou_type, class_names=None):
        ...
        if self.few_shot_mode:
            for iouThr in coco_eval.params.iouThrs:
                p = coco_eval.params
                maxDets = p.maxDets[2]
                areaRng = 'all'
                # iouThr = 0.5
                ...
                unseen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.unseen_cnames]
                seen_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.seen_cnames]
                all_cids = [p.catIds[i] for i, c in enumerate(class_names) if c in self.all_cnames]
